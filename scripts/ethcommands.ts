...
async function bridgeNativeToken(
  argv: any,
  parentChainUrl: string,
  chainUrl: string,
  inboxAddr: string,
  ...
  const bridgerParentChain = namedAccount(argv.from, argv.threadId).connect(
    argv.provider
  );
  ...
  if (argv.wait) {
    ...
    const bridger = namedAccount(argv.from, argv.threadId).connect(
      childProvider
    );
    const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));
    while (true) {
...
export const createL1ERC20Command = {
...
export const approveL1ERC20Command = {
  command: "approve-erc20-l1",
  describe: "approve simple ERC20 on L1",
  builder: {
    from: {
      string: true,
      describe: "account (see general help)",
    ...
    wait: {
      boolean: true,
      describe: "wait till l1 has approve",
      default: false,
    },
  },
  handler: async (argv: any) => {
    const l1l2tokenbridge = JSON.parse(
      fs
        .readFileSync(
          path.join(consts.tokenbridgedatapath, "l1l2_network.json")
        )
        .toString()
    );
    const deploydata = JSON.parse(
    ...
    const token = new Contract(nativeTokenAddr, ERC20.abi, deployerWallet);

    await (
      await token.functions.approve(
        l1l2tokenbridge.l2Network.tokenBridge.l1ERC20Gateway,
        // l1l2tokenbridge.l2Network.ethBridge.inbox,
        // fromAddress,
  ...
  handler: async (argv: any) => {
    if (argv.bridgeable) {
      ...
      const l1GatewayRouter = new ethers.Contract(
        l1l2tokenbridge.l2Network.tokenBridge.l1GatewayRouter,
        L1GatewayRouter.abi,
        deployerWallet
      );
      await (
      ...
      await (
        await l1GatewayRouter.functions.outboundTransfer(
          token.address,
          namedAccount(argv.mintTo).address,
          ethers.utils.parseEther("100000000"),
          100000000,
      ...
      const tokenL2Addr = (
        await l1GatewayRouter.functions.calculateL2TokenAddress(token.address)
      )[0];
...
export const transferL1ERC20Command = {
  command: "transfer-erc20-l1",
  describe: "transfers ERC20 token on L1",
  builder: {
    token: {
      string: true,
  ...
  handler: async (argv: any) => {
    ...
    await (
      await tokenContract.transfer(
        namedAccount(argv.to).address,
        ethers.utils.parseUnits(argv.amount, decimals)
      )
    ).wait();
    argv.provider.destroy();
  },
...
export const balanceOfL1ERC20Command = {
  command: "balanceOf-erc20-l1",
  describe: "balanceOf ERC20 token on L1",
  builder: {
    token: {
      string: true,
      describe: "token address",
  ...
  handler: async (argv: any) => {
    argv.provider = new ethers.providers.WebSocketProvider(argv.l1url);
    const account = namedAccount(argv.from).connect(argv.provider);
    const accountAddress = namedAccount(argv.from).address;
    const tokenContract = new ethers.Contract(argv.token, ERC20.abi, account);
    const balance = await tokenContract.balanceOf(accountAddress);
    console.log(
      `token address: ${argv.token}, account: ${accountAddress}, balanceOf: ${balance}`
    );
    argv.provider.destroy();
  },
};
